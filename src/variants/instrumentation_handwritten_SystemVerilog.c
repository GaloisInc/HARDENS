...
uint8_t Is_Ch_Tripped(uint8_t mode, uint8_t trip)
{
    ...
    uint32_t res = read_reg(INSTRUMENTATION_HANDWRITTEN_REG_RESULT);
    DEBUG_PRINTF(("<instrumentation_handwritten_SystemVerilog.c> Is_Ch_Tripped: base=0x%X, res=0x%X\n",val,res));
...
uint8_t Generate_Sensor_Trips(uint32_t vals[3], uint32_t setpoints[3])
{
    ...
    uint32_t res = read_reg(INSTRUMENTATION_HANDWRITTEN_REG_RESULT);
    DEBUG_PRINTF(("<instrumentation_handwritten_SystemVerilog.c> Generate_Sensor_Trips: vals=[%u,%u,%u], setpoints=[%u,%u,%u], res=0x%X\n",vals[0],vals[1],vals[2],setpoints[0],setpoints[1],setpoints[2],res));
