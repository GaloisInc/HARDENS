...
int instrumentation_step_trip(uint8_t div,
  ...
  int err = 0;
  /* get state */
  if ((err = read_channel(div, T, &state->reading[T])) < 0)
    return err;
  if ((err = read_channel(div, P, &state->reading[P])) < 0)
    return err;
...
int instrumentation_handle_command(uint8_t div,
  ...
  err = read_instrumentation_command(div, &i_cmd);
  if (err > 0) {
...
int instrumentation_step(uint8_t div, struct instrumentation_state *state) {
  ...
  err = instrumentation_handle_command(div, state);
  if (err < 0)
    return err;
