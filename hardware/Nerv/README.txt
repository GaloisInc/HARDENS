...
nerv.sv) into a BSV harness ("SoC"), and how to build and run it using
Verilator.

Our BSV module hierarchy is:  mkTop/ mkNervSoC/ mkNerv/ mkNerv_BVI/ nerv
where only the bottem (nerv) is in SystemVerilog (nerv.sv in repo)
and all the layers above are in BSV, in directory 'src_BSV/'

Module mkNervSoC (in file src_BSV/NervSoC.bsv)
reproduces, BSV, the nerv repo's SystemVerilog file nervsoc.sv.
...
- Top.bsv is testbench that instantiates NervSoC and just prints the
    LED values output from NervSoc whenever they change.
...
will invoke 'bsc' (open-source Bluespec compiler) on the BSV code to
generate Verilog in a new 'verilog/' directory: mkNervSoC.v, mkkNerv.v mkTop.v

    $ make link_Verilator

will use Verilator to compile and link the above Verilog files
together with nerv.sv from the nerv repo, into a simulation executable
'exe_Verilator'.  Please see the Makefile for the detailed steps.
...
We can recreate this from firmware.c and firmware.s with the following
command (we need to have a risc-v gcc compiler in our path):
...
program for this (many are available on the Internet).  Note:
  - Each entry represents a 32-bit word (not an 8-bit byte)
  - It should start at address 0 (which is the default without any '@' addr line)
  - In case the memhex32 file has address lines they should be 32-bit word addresses (not byte addresses).
...
A transcript of about the first 100 lines is in 'transcript_exe_Verilator'
